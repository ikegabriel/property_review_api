...
class ReviewCreateView(APIView):
    ...
    def post(self, request):
        ...
        if data['property_image'] == '' or None:
            del data['property_image']
        if data['property_video'] == '' or None:
            del data['property_video']
...
def country_search(request, key):
    try:
        reviews = Review.objects.filter(country=key)
    except:
...
def state_search(request, key):
    try:
        reviews = Review.objects.filter(state=key)
    except:
        return Response({'detail':'Keyword produced no results'}, status=404)
    serializer = ReviewListSerializer(reviews, many=True)
    return Response(serializer.data, status=200)
...
def city_search(request, key):
    try:
        reviews = Review.objects.filter(city=key)
    except:
        return Response({'detail':'Keyword produced no results'}, status=404)
    serializer = ReviewListSerializer(reviews, many=True)
    return Response(serializer.data, status=200)